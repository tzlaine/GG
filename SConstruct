...
if str(Platform()) == 'posix':
    ...
    options.Add('incdir', 'Location to install headers', '$prefix/include')
    options.Add('bindir', 'Location to install executables', '$prefix/bin')
    options.Add('libdir', 'Location to install libraries', '$prefix/lib')
    ...
    if not env.GetOption('clean'):
        if not preconfigured:
            ...
            if not conf.CheckBoost('1.32.0', boost_libs, conf):
                Exit(1)
            ...
            if not env['disable_sdl'] and not conf.CheckSDL(options, conf, sdl_config):
                Exit(1)

            # SDL_net (optional)
            if not env['disable_net'] and not env['with_builtin_sdlnet']:
                AppendPackagePaths('sdlnet', env)
                if not conf.CheckCHeader('SDL/SDL_net.h') or \
            ...
            if not conf.CheckCHeader('IL/il.h') or \
                ...
                Exit(1)
            if not conf.CheckLib('IL', 'ilInit') or \
...
"""
            ...
            try:
                in_f = open('include/GGConfig.h.in', 'r')
                out_f = open('include/GGConfig.h', 'w')
    ...
    result_objects, result_headers = SConscript('src/SConscript')
    gigi_objects = result_objects
    install_headers = result_headers
    result_objects, result_headers = SConscript('libltdl/SConscript')
    gigi_objects += result_objects

    # define libGiGiSDL objects
    if not env['disable_sdl']:
        result_objects, result_headers = SConscript('src/SDL/SConscript')
        gigi_sdl_objects = result_objects
        install_headers.append(result_headers)

    # define libGiGiNet objects
    if not env['disable_net']:
        result_objects, result_headers = SConscript('src/net/SConscript')
    ...
    if env['dynamic']:
        ...
        if not env['disable_sdl']:
            lib_gigi_sdl = env.SharedLibrary('GiGiSDL', gigi_sdl_objects)
        if not env['disable_net']:
            lib_gigi_net = env.SharedLibrary('GiGiNet', gigi_net_objects)
    else:
        lib_gigi = env.StaticLibrary('GiGi', gigi_objects)
        if not env['disable_sdl']:
            lib_gigi_sdl = env.StaticLibrary('GiGiSDL', gigi_sdl_objects)
        if not env['disable_net']:
            lib_gigi_net = env.StaticLibrary('GiGiNet', gigi_net_objects)
    ...
    header_dir = os.path.join(env.subst(env['incdir']), 'GG')
    lib_dir = env.subst(env['libdir'])

    # install target
    Alias('install', InstallHeaderTree(header_dir, 'include', install_headers, [], Install))
    Alias('install', Install(header_dir, 'libltdl/ltdl.h'))
    Alias('install', Install(header_dir, 'libltdl/config.h'))
    ...
    deletions = [
        Delete(header_dir),
        Delete(os.path.join(lib_dir, str(lib_gigi[0])))
        ]
    if not mising_pkg_config:
        deletions.append(Delete(os.path.join(env['pkgconfigdir'], str(gigi_pc[0]))))
    if not env['disable_sdl']:
        deletions.append(Delete(os.path.join(lib_dir, str(lib_gigi_sdl[0]))))
        if not mising_pkg_config:
            deletions.append(Delete(os.path.join(env['pkgconfigdir'], str(gigi_sdl_pc[0]))))
    if not env['disable_net']:
        deletions.append(Delete(os.path.join(lib_dir, str(lib_gigi_net[0]))))
        if not mising_pkg_config:
            deletions.append(Delete(os.path.join(env['pkgconfigdir'], str(gigi_sdl_pc[0]))))

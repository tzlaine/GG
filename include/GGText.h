...
/** This class is useful for static text; it is used primarily as the implementation for GG::StaticText.
   It renders text to an SDL surface using SDL_ttf and FreeType, then converts the SDL surface to an OpenGL texture.  This 
   involves the creation and destruction of a FreeType font and an SDL surface, so it would be wise to use this class \b only 
   for static text. It behaves more or less like a Texture after that (ie, real fast).  The text image is rendered in white, 
   with alpha blending used for antialiasing.  The user should set the desired text color with a call to glColor*() before any 
   call to OrthoBlit().  If Init() is called 
   after the image has already been initialized, the old rendered image is destroyed and text rendering is repeated, unless the 
   same string, font file, and point size are provided (since this would not alter the image).  Because of the behavior of Texture, 
   no initialization takes place and no memory or OpenGL texture is allocated when the string to render is "".  Since the m_image 
   member is a shared_ptr, TextImage objects can be copied (though Texture objects cannot), but each copy points to the same 
   rendered image; change one and they all change.  Only the last duplicate TextImage object that is destroyed will delete the 
   mutually held Texture object.*/
class TextImage
{
public:
   /** \name Structors */ //@{
   TextImage(const string& str, const string& font_filename, int pts); ///< ctor taking a text string, a font filename, and a font point size
   TextImage(const XMLElement& elem); ///< ctor that constructs a TextImage object from an XMLElement. \throw std::invalid_argument May throw std::invalid_argument if \a elem does not encode a TextImage object
   virtual ~TextImage() {} ///< virtual dtor
   //@}

   /** \name Accessors */ //@{
   const string&           GetText() const   {return m_text;}           ///< returns the text contents of image
   const string&           FontName() const  {return m_font_filename;}  ///< returns the filename of the font used to render the image
   int                     PointSize() const {return m_pt_sz;}          ///< returns the point size of the font used to render the image
   inline int              Width() const     {return m_image->Width();}   ///< returns the width of the \b entire texture containing the rendered image
   inline int              Height() const    {return m_image->Height();}  ///< returns the height of the \b entire texture containing the rendered image
   inline GLuint           OpenGLId() const  {return m_image->OpenGLId();}///< returns the "name" of the OpenGL texture containing the rendered text
   inline const GLfloat*   DefaultTexCoords() const {return m_image->DefaultTexCoords();} ///< returns the texture coordinates of the part of the texture that contains the text image \see GG::Texture::DefaultTexCoords()
   inline int              DefaultWidth() const  {return m_image->DefaultWidth();}        ///< returns the width of the rendered text portion of the image \b only \see GG::Texture::DefaultWidth()
   inline int              DefaultHeight() const {return m_image->DefaultHeight();}       ///< returns the height of the rendered text portion of the image \b only \see GG::Texture::DefaultWidth()
   inline void OrthoBlit(const Pt& pt1, const Pt& pt2, GLfloat* tex_coords = 0, bool enter_2d_mode = true) {OrthoBlit(pt1.x, pt1.y, pt2.x, pt2.y, tex_coords, enter_2d_mode);} ///< \see GG::Texture::OrthoBlit()
   inline void OrthoBlit(int x1, int y1, int x2, int y2, GLfloat* tex_coords = 0, bool enter_2d_mode = true) {m_image->OrthoBlit(x1, y1, x2, y2, tex_coords, enter_2d_mode);} ///< \see GG::Texture::OrthoBlit()
   inline void OrthoBlit(const Pt& pt, bool enter_2d_mode = true) {OrthoBlit(pt.x, pt.y, enter_2d_mode);} ///< \see GG::Texture::OrthoBlit()
   inline void OrthoBlit(int x, int y, bool enter_2d_mode = true) {m_image->OrthoBlit(x, y, enter_2d_mode);} ///< \see GG::Texture::OrthoBlit()

   virtual XMLElement XMLEncode() const; ///< constructs an XMLElement from a TextImage object
   //@}

   /** \name Mutators */ //@{
   bool Init(const string& str, const string& font_filename, int pts); ///< (re)initializes a TextImage
   //@}

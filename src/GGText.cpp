...
// class GG::TextImage
///////////////////////////////////////
TextImage::TextImage(const string& str, const string& font_filename, int pts) : 
    m_pt_sz(0)
{
    m_image = shared_ptr<Texture>(new Texture());
    Init(str, font_filename, pts);
}

TextImage::TextImage(const XMLElement& elem)
{
    if (elem.Tag() != "GG::TextImage")
	throw std::invalid_argument("Attempted to construct a GG::TextImage from an XMLElement that had a tag other than \"GG::TextImage\"");
   
    const XMLElement* curr_elem = &elem.Child("m_text");
    string str = curr_elem->Text();

    curr_elem = &elem.Child("m_font_filename");
...
XMLElement TextImage::XMLEncode() const
{
    XMLElement retval("GG::TextImage");
    XMLElement temp;
...
bool TextImage::Init(const string& str, const string& font_filename, int pts)
{
    bool retval = true;
    TTF_Font* font;
    if ((pts != m_pt_sz || str != m_text || font_filename != m_font_filename) && // don't re-render if it's the same contents we already have
	(font = TTF_OpenFont(font_filename.c_str(), pts))) {
	SDL_Color white = { 0xFF, 0xFF, 0xFF, 0xFF };
	SDL_Surface* text_surf = TTF_RenderText_Blended(font, str.c_str(), white);
	m_image->Init(text_surf);
...
void Font::Init(const string& font_filename, int pts, Uint32 range)
{
	...
	for (int c = low; c < high; ++c) {
		...
		if (x + glyph_bitmap.width >= BUF_WIDTH) { // start a new row of glyph images
			...
			int pow_of_2_x = power_of_two(max_x), pow_of_2_y = power_of_two(y + m_height);
...
bool Font::GenerateGlyph(FT_Face face, char ch)
{
    ...
    // load the glyph
    if (!face)
	throw FontException("In GG::Font::GetGlyphBitmap: invalid font or font face");

    FT_UInt index = FT_Get_Char_Index(face, ch);
    if (index) {
	if (FT_Load_Glyph(face, index, FT_LOAD_DEFAULT))
	    throw FontException((string("In GG::Font::GetGlyphBitmap: Freetype could not load the glyph for character \'") + ch) + "\'");

	FT_GlyphSlot glyph = face->glyph;

	// render the glyph
	if (FT_Render_Glyph(glyph, ft_render_mode_normal))
	    throw FontException((string("In GG::Font::GetGlyphBitmap: Freetype could not render the glyph for character \'") + ch) + "\'");
    } else {
	retval = false;
    }

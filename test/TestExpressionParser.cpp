...
BOOST_AUTO_TEST_CASE( adam_expression_parser )
{
    ...
    for (std::size_t i = 0; i < expressions.size(); ++i) {
        if (!expressions[i].empty()) {
            if (TestExpression(tok, parser_rules, stack, &adobe::parse_adam_expression, expressions[i]))
                ++passes;
            else
                ++failures;
        }
...
BOOST_AUTO_TEST_CASE( eve_expression_parser )
{
    ...
    for (std::size_t i = 0; i < expressions.size(); ++i) {
        if (!expressions[i].empty()) {
            if (TestExpression(tok, parser_rules, stack, &adobe::parse_eve_expression, expressions[i]))
                ++passes;
            else
                ++failures;
        }

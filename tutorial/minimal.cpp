...
// The application is assumed to be in "3D mode" (non-orthographic) whenever GG is not rendering "in 2D" (orthographic,
// with coordinates mapped 1-1 onto the pixels of the display).  To move into "2D" mode, GG calls Enter2DMode() at the
// beginning of each GG rendering iteration.  This method is responsible for preserving any state it alters, then
// altering the necessary state to match what GG requires.  GG requires GL_TEXTURE_2D to be on, GL_LIGHTING off, and
// requires a viewport and orthographic projection as shown below.  The other code you see here is usually convenient to
// working with GG, but not required.
void MinimalGGApp::Enter2DMode()
{
    ...
    // This sets up the world coordinate space with the origin in the upper-left corner and +x and +y directions right
    // and down, respectively.  Note that this call leaves the depth of the viewing volume is only 1 (from 0.0 to 1.0),
    // which is fine for GG's purposes.
...
void MinimalGGApp::Render()
{
    ...
    // DeltaT() returns the time in whole milliseconds since the last frame was rendered (in other words, since this
...
// should set up the initial "3D" environment, the one that exists before Enter2DMode() and after Exit2DMode().  As with
// Enter- and Exit2DMode(), the needed code is entirely application-dependent.  Note that this method is called before
...
// This is the launch point for your GG app.  This is where you should place your main GG::Wnd(s) that should appear
...
// This gets called as the application is exit()ing, and as the name says, performs all necessary cleanup at the end of
// the app's run.
...
int main(int argc, char* argv[])
{
    ...
    // The try-catch block is not strictly necessary, but it sure helps to see what exception crashed your app in the
    // log file.
    try {
